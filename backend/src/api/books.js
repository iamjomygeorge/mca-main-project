...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const authorResult = await pool.query('SELECT id FROM authors WHERE user_id = $1', [req.user.userId]);
      if (authorResult.rows.length === 0) {
        return res.status(404).json({ error: 'Author profile not found for this user.' });
      }
...
router.delete('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    if (role !== 'ADMIN' && bookAuthorUserId !== userId) {
      return res.status(403).json({ error: 'Forbidden: You are not authorized to delete this book.' });
    }
